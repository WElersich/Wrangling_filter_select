...
```
*Big Fish*, *Forrest Gump*, *O Homem Que Copiava*, and *The Incredibles* are the only four movies in the database that meet these conditions.
...
```
The first three *Harry Potter* movies, *Sorcerer's Stone*, *Chamber of Secrets*, and *Prisoner of Azkaban* are in this database. *Prisoner of Azkaban* has the highest rating, while having the lowest amount of votes. *Sorcerer's Stone* is in the reverse situation, having the most votes, but the lowest rating. *Chamber of Secrets* is in the middle of both of these categories. 

4. What is the highest budget for a movie that is an Animation or Comedy with at least 100 votes?

```{r}
animation_comedy <- movies %>%
...
animation_comedy
```
The highest budget of any animation or comedy belongs to *Wild Wild West* with a budget of 170 million dollars ($170000000).

5. Is your favorite movie in the dataset? What other questions could you ask or answer using `filter()`, `select()`, and `arrange()`?

```{r}
movies %>%
  filter(str_detect(title, "Doctor Strangelove"))

movies %>%
  filter(str_detect(title, "How I learned to stop worrying and love the bomb"))
```
Unfortunately, my favorite movie is not in the database. How disappointing. Any question concerning the data contained within the database can be answered through the use of `filter()`, `select()`, and `arrange()`.
